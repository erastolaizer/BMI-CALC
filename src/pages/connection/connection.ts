          ...
          },
          {
            ...
            handler: () => {
              this.bluetoothSerial.connect(address).subscribe((success) =>{
                this.bluetoothSerial.write('5').then((data1)=>{
                  this.bluetoothSerial.available().then (numb => {
                    this.bluetoothSerial.subscribeRawData().subscribe((rawData) => {
                       this.bluetoothSerial.read().then(data =>{

                      this.measure1 = data;
                      this.sendNotification('Data received, ' + this.measure1);
                      this.bluetoothSerial.disconnect();
                    }, err =>{
                      this.sendNotification('Data not received,');
                      this.bluetoothSerial.disconnect();
